...
- **UX tests (XCUITest)**: **`cd Leo && ./scripts/run-ux-tests.sh`** — runs **`build-app.sh`** then **`xcodebuild test`** on **`Leo/XcodeUX/LeoUX.xcodeproj`** (scheme **`LeoUX`**). Launches **`Leo/build/LeoUITest.app`** by path; grant **Accessibility** to the test runner if macOS blocks automation.
...
- **Build app**: `cd Leo && ./build-app.sh` → `Leo/build/LeoComposer2.app` (product **LeoComposer2**; override with `LEO_BUNDLE_STEM_OVERRIDE`). Open that build, not only `swift run`, for real `Bundle.main` + embedded `web/`
...
- Do not delegate trivial verification (rebuild, `swift test`, opening a fresh `.app`, Xcode MCP runs) to the user when the agent can execute those steps.
...
- **Leo / Foliate bridge:** `JSONSerialization` with JSON data requires a top-level array or dictionary; serializing a bare `String` crashes. When bridging string payloads from Swift to the WKWebView (e.g. `FoliateReaderView`), wrap strings in a single-element array (or use a dictionary) before UTF-8 encoding, then strip or parse on the JS side as needed.
