...
The ElevenLabs Swift SDK can start a session with a **public** `agent_id` only for agents marked public in the dashboard. **Private** agents need a **short-lived conversation token** minted with your **API key**. That key must not be embedded in `VoiceTutor.app`.
...
4. In **Voice Tutor → Settings**, enable **Use token broker**, set **Broker URL** to `http://127.0.0.1:8787`, and set your **Agent ID**.
